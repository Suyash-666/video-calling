...
export function useWebRTC(): UseWebRTCResult {
  ...
  const createPeerFor = useCallback(
    (peerId: string, localMedia: MediaStream): PeerEntry => {
      ...
      // CRITICAL: create the data channel BEFORE addTrack so the SDP
      // generated by the first createOffer() already includes the
      // data-channel m-line. Subsequent offers (e.g. iceRestart) must
      // keep the same m-line order — adding the channel later would
      // shift the order and the browser would reject the new offer
      // with "The order of m-lines in subsequent offer doesn't match".
      ...
      // The browser will have set a fresh localDescription on the PC
      // by the time this fires (the implicit createOffer/setLocalDescription
      // cycle that comes with restartIce). We just read it and send.
      // Crucially, we don't call createOffer() ourselves — that would
      // produce an SDP whose m-line order might not match the
      // answerer's view, causing "The order of m-lines in subsequent
      // offer doesn't match". The fix for THAT was to create the
      // data channel BEFORE the first addTrack, so the m-line order
      // is stable from the very first negotiation.
      pc.onnegotiationneeded = async () => {
        if (selfId >= peerId) return; // answerer side doesn't offer
        if (pc.signalingState !== 'stable') return; // mid-flight, drop
